...
class CRM_Committees_Implementation_PersonalOfficeSyncer extends CRM_Committees_Plugin_Syncer {
  ...
  public function syncModel($model, $transaction = TRUE) {
    ...
    foreach ($model->getAllPersons() as $active_person) {
      ...
      if ($po_contact_id) {
        $all_active_po_contact_ids[] = $po_contact_id;
      }
    }
    ...
    foreach ($new_addresses as $address) {
      ...
      if (!empty($address_data['supplemental_address_1'])) {
        $address_string .= "|{$address_data['supplemental_address_1']}";
      }
      if (!empty($address_data['postal_code'])) {
        $address_string .= "|{$address_data['postal_code']}";
      }
      if (!empty($address_data['city'])) {
        $address_string .= "|{$address_data['city']}";
      }
    ...
    foreach ($changed_addresses as $changed_address) {
      ...
      if ($person) {
        ...
        foreach ($differing_attributes as $differing_attribute) {
          ...
          if ($differing_attribute == 'supplemental_address_1') {
            continue;
          }
    ...
    // run diff
    // todo: fine-tune
    $ignore_attributes = ['relationship_id', 'relationship_type_id', 'start_date', 'committee_name', 'description', 'represents', 'employee_contact_id', 'employer_contact_id'];
